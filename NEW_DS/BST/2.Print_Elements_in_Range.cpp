...
void elementsInRangeK1K2(int k1, int k2, BinaryTreeNode<int>* root) {
  if(root == nullptr) return; 
    if(root -> data > k2){
        elementsInRangeK1K2(k1, k2, root->left); 
    }else if(root -> data < k1){
        elementsInRangeK1K2(k1, k2, root -> right); 
    }else{
        elementsInRangeK1K2(k1, k2, root -> left ); 
        cout << root -> data << " "; 
        elementsInRangeK1K2(k1, k2, root -> right); 
...
int main() {
    ...
    int k1, k2;
    cin >> k1 >> k2;
    elementsInRangeK1K2(k1, k2, root);
}